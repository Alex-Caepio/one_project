...
class UpdateRequest extends Request {
    ...
    public function rules() {
        ...
        return [
            ...
            'cancel_bookings_on_unpublish' => [
                'sometimes', 'bool',
                Rule::requiredIf($user->isPractitioner() && $user->is_published === true &&
                                 $user->bookings()->exists() && $this->is_published === false)
            ],
            'business_country_id'             => [
                'nullable',
                Rule::requiredIf($user->isPractitioner()),
                'integer',
                'exists:countries,id'
            ],
        ...
        $validator->after(function($validator) use ($user) {
            if ($this->getBoolFromRequest('is_published') === true || $user->is_published) {
                ...
                if (!$user->business_time_zone_id && !$this->business_time_zone_id) {
                    $validator->errors()->add('business_time_zone_id', 'You have not filled in the field "Timezone"');
                }
            }
        });
        $validator->after(function($validator) use ($user) {
